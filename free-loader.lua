...
local KEY_LINK       = "https://ads.luarmor.net/get_key?for=NoxHub_Free-YgsLsSacLsra"
...
local function Toast(text, ok)
    ...
    TweenService:Create(card, TweenInfo.new(0.22, Enum.EasingStyle.Quad), {
        Position = UDim2.new(1, 0, 0, 0), Transparency = 0
    }):Play()

    task.delay(2.3, function()
        if card.Parent then
            TweenService:Create(card, TweenInfo.new(0.22, Enum.EasingStyle.Quad), {
                Position = UDim2.new(1, 380, 0, 0), Transparency = 0.05
            }):Play()
            task.wait(0.24)
            card:Destroy()
        end
...
shine.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0.00, Color3.fromRGB(24, 26, 46)),
    ColorSequenceKeypoint.new(0.60, Color3.fromRGB(18, 20, 38)),
    ColorSequenceKeypoint.new(1.00, Color3.fromRGB(14, 16, 30))
}
...
local function updateTwo()
    local v = Camera and Camera.ViewportSize or Vector2.new(1280,720)
    if v.X < 900 then
        grid.CellSize = UDim2.new(1, 0, 1, 0)
        twoRow.Size = UDim2.new(1, -4, 0, 104)
    else
        grid.CellSize = UDim2.new(0.5, -6, 1, 0)
        twoRow.Size = UDim2.new(1, -4, 0, 48)
    end
...
local btnDiscord = makeButton("Copy Discord"); btnDiscord.Parent = twoRow
local btnGetKey  = makeButton("Copy Get Key");  btnGetKey.Parent  = twoRow
...
btnDiscord.MouseButton1Click:Connect(function()
    if has_clip then setclip(DISCORD_LINK); Toast("Discord link copied.", true)
    else Toast("Clipboard unsupported. Printed to console.", false); print("[NoxHub] Discord:", DISCORD_LINK) end
end)

btnGetKey.MouseButton1Click:Connect(function()
    if KEY_LINK == "KEY_LINK_PLACEHOLDER" then Toast("Replace KEY_LINK with your real URL.", false); return end
    if has_clip then setclip(KEY_LINK); Toast("Get Key link copied.", true)
    else Toast("Clipboard unsupported. Printed to console.", false); print("[NoxHub] Get Key:", KEY_LINK) end
end)
